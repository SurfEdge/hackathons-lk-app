...
class EndedEventsPostsController extends GetxController {
  ...
  Future<List<Data>> fetchData(int pageNumber) async {
    final response = await http.get(Uri.parse(
        'https://hackathons.lk/wp-json/wp/v2/event?_embed&page=$pageNumber&per_page=$perPage'));
  ...
  fetchPosts({int pageNumber = 0}) async {
    try {
      ...
      if (eventsList.length < totalRecords.value) {
        ...
        if (posts != null) {
          eventsList.addAll(posts.where((i) => i.status == 'Ended').toList());
          perPage(5);
          print(eventsList.length);

          if (eventsList.length < 3) {
            var posts = await fetchData(pageNumber + 1);
            if (posts != null) {
